...
public class OrderedItemsAdapter extends RecyclerView.Adapter<OrderedItemsAdapter.OrderedItemsViewHolder> {
    Context context;
    List<Datum> userOrderDetailList;
    private static final String TAG = "OrderedItemsAdapter";

    public OrderedItemsAdapter(Context context, List<Datum> userOrderDetailList) {
    ...
    public void onBindViewHolder(@NonNull final OrderedItemsViewHolder holder, int position) {
        final Datum datum = userOrderDetailList.get(position);
        ...
        holder.product_quantity.setText("Qty:" + datum.getQuantity());
        holder.product_seller.setText("Sold By: "   );

        if(datum.getImage()!=null && !isEmpty(datum.getImage())){
            Picasso.get().load(datum.getBaseUrl()+datum.getImage()).placeholder(R.drawable.app_icon_png).fit().into(holder.product_img);
        }

        holder.product_price.setText(context.getString(R.string.rupee_icon)+datum.getTxnAmount());
    ...
    private void cancelOrder(Datum datum) {
            ...
            Call<ResponseModel> loginResponseCall = getApiInstance().cancelOrder(getCurrentUser().getId(), getCurrentUser().getAccessToken(), datum.getId());
...
//        alertDialog.setIcon(R.drawable.ic_logout);
    ...
    public void updateItems(List<Datum> userOrderDetailList){
